...
export function drawTextCellWithAlignment(
  ...
  const fontFamily = isCJK ? 
    '"Noto Sans TC", "Microsoft YaHei", "微軟雅黑", "SimHei", "黑体", sans-serif' : 
    '"Arial", sans-serif';
  
  // Adjust font size for CJK text if needed
  const adjustedFontSize = isCJK ? Math.round(fontSize * 0.9) : fontSize;
